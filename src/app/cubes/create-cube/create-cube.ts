...
@Component({
  ...
  templateUrl: './create-cube.html',
  styleUrl: './create-cube.scss',
})
export class CreateCube implements OnInit {
  ...
  form: FormGroup;
  cubeNameControl = new FormControl<string>('', [
    Validators.required,
    Validators.minLength(3),
    Validators.pattern(/^[a-zA-Z ]*$/),
  ]);
  cubeUrlControl = new FormControl<string>('', [Validators.required]);
  numCardsControl = new FormControl<number | undefined>(450, [
    Validators.min(90),
    Validators.max(9999),
  ]);
  shortDescriptionControl = new FormControl<string | undefined>(undefined, [
    Validators.minLength(10),
    Validators.maxLength(90),
  ]);
  longDescriptionControl = new FormControl<string>('', [
    Validators.minLength(10),
  ...
  constructor(
    ...
    this.form = this.formBuilder.group({
      name: this.cubeNameControl,
      url: this.cubeUrlControl,
      numCards: this.numCardsControl,
  ...
  ngOnInit() {
    ...
    this.f['name'].valueChanges
      .pipe(
        tap((name) => {
          if (name) {
            this.cubeUrlControl.setValue(name.replaceAll(' ', ''));
          }
        }),
      )
      .subscribe();
  ...
  clearNumCards() {
    this.numCardsControl.patchValue(undefined);
  }
